...
//if (settingsREAD.autoread2 == 'true') await this.readMessages([m.key]);
 ...
 if (db.data.chats[m.chat].reaction && m.text.match(/(ción|dad|aje|oso|izar|mente|pero|tion|age|ous|ate|and|but|ify|yeray|bot|saku|bug|syntax)/gi)) {
  ...
  for (const nk of callUpdate) {
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        ...
        const vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;Creador 🥷🏼;;;\nFN:Creador 🥷🏼\nORG:Creador 🥷🏼\nTITLE:\nitem1.TEL;waid=593992402778:+50588586015/nitem1.X-ABLabel:Creador 🥷🏼Creador 🥷🏼\nX-WA-BIZ-DESCRIPTION:ᴄᴏɴᴛᴀᴄᴛᴀ ᴀ ᴇsᴛᴇ ɴᴜᴍ ᴘᴀʀᴀ ᴄᴏsᴀs ɪᴍᴘᴏʀᴛᴀɴᴛᴇs.\nX-WA-BIZ-NAME:Creador 🥷🏼\nEND:VCARD`;
        await mconn.conn.sendMessage(nk.from, {contacts: {displayName: 'Best𝘉𝘰𝘵 ✨', contacts: [{vcard}]}}, {quoted: callmsg});
